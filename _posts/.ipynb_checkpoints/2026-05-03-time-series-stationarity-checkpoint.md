...
### 3.2 약정상성(Weak / Covariance Stationarity)
...
## 4. 왜 비정상 시계열은 분석/예측이 어려운가?

비정상 시계열을 그대로 고전 모델에 투입했을 때 발생하는 문제는 크게 두 가지입니다.

### 4.1 허구적 회귀(Spurious Regression)
...
### 5.1 ADF 검정 (Augmented Dickey-Fuller Test)
...
## 6. 어떤 성분이 비정상성을 유발하며, 어떻게 정상화하는가?

### 6.1 추세(Trend) → 차분 또는 추세 제거
...
### 6.3 분산의 비균일성(Heteroscedasticity) → 변환

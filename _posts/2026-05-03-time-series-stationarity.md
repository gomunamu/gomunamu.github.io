...
![AR(1) 동작](/assets/img/posts/fig_ar1.png)

같은 노이즈 $\epsilon_t$를 줘도 $\phi$만 바꾸면 결과가 이렇게 달라집니다.

- $\phi = 0.3$: 직전 값의 영향이 약해 빠르게 0 부근으로 회귀합니다. 충격은 곧 사라집니다.
- $\phi = 0.9$: 영향이 천천히 사라져 굵은 출렁임이 보이지만, 여전히 평균 0 주변을 맴돕니다.
- $\phi = 1.0$: 충격이 사라지지 않고 누적됩니다. 랜덤워크입니다. 평균이 0 주변에 머문다는 보장이 없어집니다.
- $\phi = 1.05$: 직전 값보다 더 크게 반영되어 시계열이 발산합니다.

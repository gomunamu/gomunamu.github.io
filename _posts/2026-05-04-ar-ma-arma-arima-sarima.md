...
> 
> 그럼 $\theta = 1$일 때 무엇이 깨지는가? **가역성(invertibility)** 입니다. $\lvert \theta \rvert < 1$인 가역적 MA(1)은 무한차수 AR로 다시 쓸 수 있습니다 — 즉 관측된 시계열로부터 과거 충격(잔차)들을 거꾸로 복원할 수 있어요. 이게 예측에서 결정적인 성질입니다. $\theta = 1$이면 이 복원이 불가능해지고, 게다가 $\theta$와 $1/\theta$인 두 모형이 통계적으로 구분되지 않는 식별 문제도 생깁니다.[^3]
> 

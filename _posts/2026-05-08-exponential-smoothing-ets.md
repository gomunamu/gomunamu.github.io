...
> **함정 박스 3 — 다단계 예측의 신뢰구간은 시간이 갈수록 넓어진다**
> 
> 다단계 예측은 본질적으로 **재귀적**입니다. $\hat{y}\_{T+1}$의 불확실성이 $\hat{y}\_{T+2}$ 계산에 누적되고, 그게 다시 $\hat{y}\_{T+3}$에 누적되는 식이지요. 그래서 신뢰구간이 시간이 갈수록 발산하듯 넓어집니다. 이전 SARIMA 글에서 본 신뢰구간 발산과 같은 메커니즘입니다. ETS는 상태공간 모형이라 이 누적이 통계적으로 정확히 정량화됩니다 — `get_prediction(steps=h).conf_int()`로 확인하실 수 있습니다.

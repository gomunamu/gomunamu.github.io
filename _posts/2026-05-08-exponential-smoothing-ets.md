...
여기까지 따라오신 분이라면 한 가지 의문이 자연스럽게 드실 겁니다. **이름이 "평활"인데 정말 예측 도구인가? 두 시점 앞을 예측하려면 어떻게 하는가?**
...
이 다단계 예측의 모양 차이가 사실 **컴포넌트를 추가한 진짜 이유**입니다. 단순히 "더 잘 맞아서"가 아니라, **추세 컴포넌트가 있어야 우상향이 가능하고, 계절 컴포넌트가 있어야 톱니 패턴이 가능**하니까요. SES → Holt → Holt-Winters의 빌드업은 결국 "예측 곡선의 모양을 더 풍부하게 만드는 과정"입니다.

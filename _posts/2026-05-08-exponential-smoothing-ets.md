...
레벨 식에서 눈에 띄는 부분은 $(1-\alpha)$ 항이 $\ell\_{t-1}$이 아닌 $\ell\_{t-1} + b\_{t-1}$이라는 점입니다. **$\ell\_{t-1} + b\_{t-1}$은 직전 시점에서 $y\_t$를 예측한 값** $\hat{y}\_{t|t-1}$입니다. 따라서 레벨 업데이트는 "실제 관측값"과 "한 시점 전에 예측했던 값"을 $\alpha : (1-\alpha)$로 섞는 구조입니다.

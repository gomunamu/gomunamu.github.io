...
> **용어 혼동 — `rolling().mean()`과 ARIMA의 MA는 다릅니다**
>
> 둘 다 "이동평균"이라는 이름을 공유하지만 개념이 전혀 다릅니다.
>
> `rolling(k).mean()`은 **관측값** $k$개의 단순 평균입니다. ARIMA의 MA(q)는 그게 아니라 **과거 오차항(충격)의 가중합**입니다.
>
> $$y_t = \varepsilon_t + \theta_1\varepsilon_{t-1} + \cdots + \theta_q\varepsilon_{t-q}$$
>
> "이동평균"이라는 이름은 Slutsky(1927)에서 유래합니다. 그는 순수한 백색잡음에 이동평균을 반복 적용하면 없던 주기성이 나타난다는 걸 보였습니다 — 이 아이디어를 수식화한 것이 MA 모형입니다. 현재 값은 과거 충격들이 현재까지 흘러들어온 누적 효과이고, 그 충격들의 이동평균이라는 뜻입니다. `rolling().mean()`이 더 나중에 같은 이름을 쓰게 된 탓에 혼동이 생겼습니다.

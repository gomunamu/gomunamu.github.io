...
**Laplace 사전분포로 대부분의 후보를 소거**

25개 후보를 모두 활성화하면 과적합이므로, $\boldsymbol{\delta}$에 Laplace 사전분포를 줍니다:

$$
\delta_j \sim \text{Laplace}(0,\, \tau), \quad \tau = \texttt{changepoint\_prior\_scale}\ (\text{기본값 } 0.05)
$$

Laplace 분포는 0에서 날카롭게 집중됩니다. MAP 추정 관점에서 이는 **LASSO(L1 정규화)와 수학적으로 동치**라, 작은 $\delta_j$를 정확히 0으로 만들어 대부분의 후보를 자동 소거합니다. $\tau$가 작을수록 더 많이 소거되어 추세가 완만해지고, 클수록 더 많은 change point가 살아남아 추세가 유연해집니다.[^3]

<details>
<summary>Laplace 사전분포와 정규분포 비교</summary>

...
**MAP(Maximum A Posteriori) 추정**이란 사전분포(prior)와 데이터 likelihood를 결합한 사후확률(posterior)을 최대화하는 파라미터를 찾는 방법입니다. Bayes 정리에 따르면:

$$
p(\boldsymbol{\delta} \mid \mathbf{y}) \propto p(\mathbf{y} \mid \boldsymbol{\delta})\, p(\boldsymbol{\delta})
$$

최대화는 로그를 취해 합산으로 바꾸면 다루기 쉽습니다:

$$
\hat{\boldsymbol{\delta}}_{\text{MAP}} = \arg\max_{\boldsymbol{\delta}}\,\bigl[\log p(\mathbf{y} \mid \boldsymbol{\delta}) + \log p(\boldsymbol{\delta})\bigr]
$$

사전분포 $p(\boldsymbol{\delta})$가 없으면 MLE(최대 우도 추정)와 같고, 있으면 사전분포가 정규화 항처럼 작동해 파라미터를 특정 방향으로 수축시킵니다. Prophet의 경우 핵심 항만 단순화해서 MAP 추정의 목적함수를 전개하면:

...
$$
\delta_j = \beta_j - \beta_{j-1} \quad \text{(인접 구간의 기울기 차이)}
$$

이렇게 두면 Laplace 사전분포가 딱 맞아 떨어집니다. $\delta_j = 0$은 "이 change point에서 기울기 변화 없음"을 직접 의미하므로, Laplace$(0, \tau)$로 $\delta_j$를 0 쪽으로 수축시키는 것이 곧 "대부분의 change point를 비활성화한다"는 뜻이 됩니다.

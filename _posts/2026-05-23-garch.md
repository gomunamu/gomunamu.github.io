...
GARCH의 파라미터(ω, α, β)는 **최대가능도추정(MLE)**으로 구합니다([가능도 개념]({% post_url 2026-05-02-parameter-estimation %})). 수익률 잔차는 정규분포보다 꼬리가 두꺼운 경우가 많아 **t-분포**를 오차 분포로 씁니다. MLE로 구한 로그가능도는 모형 복잡도를 보정한 **AIC**로 변환해 어느 모형이 더 나은지 비교합니다.

...
\frac{
  ...
  \underbrace{p(\mathbf{y} \mid X)}_{\text{주변우도(evidence)}}
}
$$

각 항의 역할을 정리하면 다음과 같습니다.

- **사전 분포 $p(\mathbf{w})$**: 데이터를 보기 전, $\mathbf{w}$에 대한 우리의 믿음
- **우도 $p(\mathbf{y} \mid X, \mathbf{w})$**: 파라미터 $\mathbf{w}$가 주어졌을 때 이 데이터가 나올 확률
- **사후 분포 $p(\mathbf{w} \mid X, \mathbf{y})$**: 데이터를 본 뒤 업데이트된 믿음
- **주변우도 $p(\mathbf{y} \mid X)$**: $\mathbf{w}$와 무관한 정규화 상수 — 최적화에는 무시 가능

...
**빈도주의**: 파라미터 $\mathbf{w}$는 고정된 미지의 상수입니다. 데이터로부터 그 값을 추정합니다. OLS, MLE가 여기에 해당합니다.
...
잡음이 가우시안이면, 로그 가능도를 최대화하는 것이 RSS를 최소화하는 것과 같습니다.
...
라플라스는 $w=0$에서 **뾰족한 꼭짓점**을 가집니다. $-\log p(w) \propto |w|$인 V자이므로, 최적화가 $w=0$에 붙어버리는 현상이 발생합니다. 이것이 Lasso가 계수를 정확히 0으로 만드는 이유입니다.

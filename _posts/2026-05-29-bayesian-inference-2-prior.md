...
아무 정보도 없다는 표현. 하지만 "정보 없음"을 표현하는 것이 생각보다 어렵습니다.  
파라미터 변환 시 균등하지 않게 되는 문제 (Jeffreys prior가 이를 해결).

### Weakly informative prior

완전히 균등하지 않지만 매우 넓은 분포.  
예: $\theta \sim \mathcal{N}(0, 10)$ — "대충 0 근처겠지만 많이 틀려도 괜찮아"
...
Prior가 달라도 데이터가 충분하면 posterior는 거의 같아집니다.

$$
p(\theta \mid \mathcal{D}) \approx p(\theta \mid \mathcal{D}) \quad \text{(다른 prior여도)}
$$
...
Hyperparameter에도 분포를 씌우면 **hierarchical model** — 이 구조는 데이터로부터 prior 자체를 학습하는 효과를 줍니다.

ML에서 흔히 쓰는 "hyperparameter tuning"도 같은 언어입니다.  
베이지안은 그것도 분포로 다룰 수 있다고 말합니다.
...
[3편]({% post_url 2026-05-29-bayesian-inference-3-inductive-bias %})에서는 ML 모델의 아키텍처 선택이 사실 prior 설계와 같다는 이야기 — inductive bias와 prior의 연결고리를 다룹니다.
